...
export default function useNewKindergartenForm() {
  ...
  const onFinish: FormProps<NewKindergartenFieldType>["onFinish"] = (values) => {
    setFormErrors({})
    console.log(values)
    newKindergarten({
      name: values?.name,
  ...
  const onFinishFailed: FormProps<NewKindergartenFieldType>["onFinishFailed"] = (errorInfo): void => {
