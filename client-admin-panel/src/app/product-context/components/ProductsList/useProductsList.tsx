...
export default function useProductsList() {
  const [selectedProviderId, setSelectedProviderId] = useState<string>("ikonka")
  const [productsList, setProductsList] = useState<ProductsDataType[]>([])
  const [providerProductsList, setProviderProductsList] = useState<ProviderProductsDataType[]>([])
  const [triggerGetIkonkaProducts, getIkonkaProductsResult] = useLazyGetIkonkaProductsQuery()
  ...
  const onFinishFailed: FormProps<FilterProductsFieldType>["onFinishFailed"] = (errorInfo): void => {
  ...
  const handleGetIkonkaProducts = async () => {
    ...
    const result = await triggerGetIkonkaProducts({
      ...
      limit: "10",
      providerProductId: providerProductsForm.getFieldValue("providerProductId"),
    })

    if (result.isSuccess) {
      const providerProducts: ProviderProductsDataType[] = result?.data?.data?.items?.map((ikonkaProduct) => ({
  ...
  const handleNavigateToAddProviderProductToProducts = (providerProductId: string) => {
    navigate(`/${ROUTES.app}/${ROUTES.product}?providerProductId=${providerProductId}&selectedProviderId=${selectedProviderId}`)
  }
  ...
  const productsTableColumns: TableProps<ProductsDataType>["columns"] = [
    ...
    {
      title: t(`products.products-list.${PRODUCTS_TABLE_HEADERS.provider}-header`),
    ...
    {
      title: t(`products.products-list.${PRODUCTS_TABLE_HEADERS.priceChanged}-header`),
    ...
    {
      title: t(`products.products-list.${PRODUCTS_TABLE_HEADERS.updates}-header`),
    ...
    {
      title: t(`products.products-list.${PRODUCTS_TABLE_HEADERS.categoryMissing}-header`),
  ...
  const providerProductsTableColumns: TableProps<ProviderProductsDataType>["columns"] = [
    {
      title: t(`products.products-list.${PROVIDER_PRODUCTS_TABLE_HEADERS.id}-header`),
    ...
    {
      title: t(`products.products-list.${PROVIDER_PRODUCTS_TABLE_HEADERS.name}-header`),
    ...
    {
      title: t(`products.products-list.${PROVIDER_PRODUCTS_TABLE_HEADERS.providerId}-header`),
    ...
    {
      title: t(`products.products-list.${PROVIDER_PRODUCTS_TABLE_HEADERS.actions}-header`),
