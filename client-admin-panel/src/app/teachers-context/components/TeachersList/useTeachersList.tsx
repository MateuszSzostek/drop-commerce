...
export default function useTeachersList() {
  ...
  const onFinishFailed: FormProps<NewPasswordFieldType>["onFinishFailed"] = (
    errorInfo
  ): void => {
  ...
  const teachersTableColumns: TableProps<TeachersDataType>["columns"] = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Link
          ...
          to={`/${ROUTES.app}/${ROUTES.teacherPreview}/${record.id}`}
        >
          {record.name}
        </Link>
      ),
    ...
    {
      title: t(
        `teachers.teachers-list.${TEACHERS_TABLE_HEADERS.groups}-header`
      ),
      ...
      render: (_, record) => {
        return record.groups?.map((group) => (
          <Space direction="horizontal">
            <Link
              styleType={LinkStyleType.TABLE_RECORD}
              to={`/${ROUTES.app}/${ROUTES.groupPreview}/${group.id}`}
            >
              {group.name}
            </Link>{" "}
            <span> </span>
    ...
    {
      title: t(
        `teachers.teachers-list.${TEACHERS_TABLE_HEADERS.phoneNumber}-header`
      ),
